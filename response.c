...
const struct response responses[] = {
	...
	{401, "<%s> :No such nick/channel",                      response_err_send},
	{402, NULL, NULL},
	{403, "<%s> :No such channel",                           response_err_send},
	{404, NULL, NULL},
	{405, NULL, NULL},
	{406, NULL, NULL},
	{407, NULL, NULL},
	...
	{461, "<%s> :Not enough parameters",                     response_err_send},
	{462, NULL, NULL},
	{463, NULL, NULL},
	{464, NULL, NULL},
	{465, NULL, NULL},
...
int response_err_send(struct request *req, struct collection *col)
{
	...
	if (write(req->src->fd, req->body, strlen(req->body)) == -1)
		return -1;
		...
		struct collection *col)
{
	...
	p = res->desc;
	if (strcmp(p, "<nick>") == 0) {
		req->body = str_replace(p, "<nick>", req->src->nick);
		p = req->body;
	}

	if (strcmp(p, "<command>") == 0) {
		req->body = str_replace(p, "<command>", req->src->nick);
